#!/usr/bin/env python3
"""
Train, validate, and test a simple MLP baseline on windowed GraphLeak CSVs.

This script expects train/val/test CSVs produced by
graphleak_volume_build_windowed_splits.py. It keeps the workflow auditable by
writing metrics, predictions, and training history back to CSV files.
"""
...
DEFAULT_INPUT_PREFIX = "mlp/n2/graphleak_volume_windows_w24_s1_V_N2_label-leak_binary"
DEFAULT_OUTPUT_PREFIX = "mlp/n2/graphleak_volume_mlp_baseline"
DEFAULT_EPOCHS = 40
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a simple MLP baseline on GraphLeak windowed CSVs."
    )
    parser.add_argument(
        "--input-prefix",
        default=DEFAULT_INPUT_PREFIX,
        help="Common prefix of the windowed train/val/test CSVs.",
    )
    parser.add_argument(
        "--output-prefix",
        default=DEFAULT_OUTPUT_PREFIX,
        help="Common prefix for output artifacts.",
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Hidden layer sizes, e.g. --hidden-dims 64 32.",
    )
    parser.add_argument(
        "--dropout",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Early stopping patience measured in epochs without val_loss improvement.",
    )
    parser.add_argument(
        "--seed",
        type=int,
...
def main() -> None:
    ...
    train_x, val_x, test_x, mean, std = standardize_splits(
    ...
    torch.save(
        {
            ...
            "scaler_mean": mean,
            "scaler_std": std,
            "label_col": train_df["label_col"].iloc[0] if "label_col" in train_df.columns else "y_true",
        },
        model_path,
    )
